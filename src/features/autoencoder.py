...
class TimeSeriesDataset(object):
    def __init__(self, df_polars: pl.DataFrame, numerical_cols: List[str], categorical_cols: List[str],
                 seq_length: int, batch_size: int, prediction_window: int = 1):
        """
        :param df_polars: polars dataframe
        :param numerical_cols: name of the numerical columns, if None pass empty list
    ...
    @property
    def nfeatures(self):
        return len(self.numerical_cols + self.categorical_cols)
...
def evaluate_autoencoder(test_iter, criterion, model, config, ts):
    """
